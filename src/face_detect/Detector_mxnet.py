...
from tools_mxnet import nms, adjust_input, detect_first_stage_warpper,detect_first_stage
...
class MtcnnDetector(object):
    ...
    def detectFace(self, img):
        ...
        if not self.accurate_landmark:
            return rectangles
        # extended stage
        rectangles = self.Lnet_Process(rectangles,img)
        return rectangles

    def detect_faces(self,img,cropsize):
        # check input
        if img is None:
            return []
        # only works for color image
